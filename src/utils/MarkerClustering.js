 ...
 */
let MarkerClustering = function(options) {
  // 기본 값입니다.
  ...
  this.setOptions(
    window.naver.maps.Util.extend({}, this.DEFAULT_OPTIONS, options),
    true
  );
  this.setMap(options.map || null);
};

window.naver.maps.Util.ClassExtend(
  MarkerClustering,
  window.naver.maps.OverlayView,
  {
    onAdd: function() {
      var map = this.getMap();

      this._mapRelations = window.naver.maps.Event.addListener(
        map,
        'idle',
        window.naver.maps.Util.bind(this._onIdle, this)
      );
    ...
    draw: window.naver.maps.Util.noop,

    onRemove: function() {
      window.naver.maps.Event.removeListener(this._mapRelation);
    ...
    setOptions: function(newOptions) {
      ...
      if (typeof newOptions === 'string') {
        ...
        window.naver.maps.Util.forEach(newOptions, function(value, key) {
          if (key !== 'map') {
            _this.set(key, value);
          }
        });
    ...
    getOptions: function(key) {
      ...
      if (key !== undefined) {
        ...
        window.naver.maps.Util.forEach(_this.DEFAULT_OPTIONS, function(
          value,
          key
        ) {
      ...
      for (var i = 0, ii = markers.length; i < ii; i++) {
        ...
        this._markerRelations.push(
          window.naver.maps.Event.addListener(
            marker,
            'dragend',
            window.naver.maps.Util.bind(this._onDragEnd, this)
          )
        );
      }
    },
    ...
    _clearClusters: function() {
      ...
      window.naver.maps.Event.removeListener(this._markerRelations);
...
Cluster.prototype = {
  ...
  /**
   * 클러스터 중심점을 반환합니다.
   * @return {window.naver.maps.LatLng} 클러스터 중심점
   */
  getCenter: function() {
  ...
  /**
   * 클러스터 경계 영역을 반환합니다.
   * @return {window.naver.maps.LatLngBounds} 클러스터 경계 영역
   */
  getBounds: function() {
   ...
   * @return {window.naver.maps.Marker[]} 클러스터를 구성하는 마커 객체 집합
   */
   ...
   * @param {window.naver.maps.LatLng} latlng 위/경도
  ...
  enableClickZoom: function() {
    ...
    this._relation = window.naver.maps.Event.addListener(
      this._clusterMarker,
      'click',
      window.naver.maps.Util.bind(function(e) {
  ...
  _calcBounds: function(position) {
    ...
    var max_px_x = Math.min(map_max_px.x, max_px.x),
      ...
      newMax = proj.fromOffsetToCoord(
        new window.naver.maps.Point(max_px_x, max_px_y)
      ),
      newMin = proj.fromOffsetToCoord(
        new window.naver.maps.Point(min_px_x, min_px_y)
      );

    return new window.naver.maps.LatLngBounds(newMin, newMax);
  },
  ...
  _getIndex: function(count) {
    ...
    if (window.naver.maps.Util.isFunction(indexGenerator)) {
      return indexGenerator(count);
    } else if (window.naver.maps.Util.isArray(indexGenerator)) {
      var index = 0;
   ...
   * @param {window.naver.maps.Marker} marker 마커
   ...
   * @param {Array.<window.naver.maps.Marker>} markers 마커 배열
   * @return {window.naver.maps.Point} 마커들의 중심 좌표
   * @private
   */
  _calcAverageCenter: function(markers) {
    ...
    return new window.naver.maps.Point(averageCenter[0], averageCenter[1]);
  },
};
